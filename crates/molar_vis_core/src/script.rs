    ...
    match cmd {
        ...
        Command::Select { mol, rep, text } => {
            ...
            if let Err(EvalError::Invalid { message, .. }) =
                crate::scene::evaluate(&scene.molecules[mi].system, &text)
            {
                return Err(message);
            }
        ...
        Command::Focus { mol, text } => {
            ...
            let (_, sel) = crate::scene::evaluate(&m.system, &text).map_err(|e| match e {
...
/// Evaluate `source` as a Rhai script. `summary` is the pre-evaluation scene listing
/// returned by `list()`. No scene access here — only command collection + output.
pub fn evaluate_script(source: &str, summary: String) -> EvalOutcome {
    let cmds: Queue = Rc::new(RefCell::new(Vec::new()));
    ...
    // list() — print the (pre-script) scene summary.
    {
        let o = out.clone();
    ...
    if let Err(e) = engine.run(source) {
        out.borrow_mut().push(ConsoleLine { kind: LineKind::Error, text: e.to_string() });
    }
    drop(engine); // releases the registered closures' queue clones before we take it

    let commands = std::mem::take(&mut *cmds.borrow_mut());
    let output = std::mem::take(&mut *out.borrow_mut());
...
mod tests {
    ...
    fn commands_mutate_the_scene() {
        ...
        crate::scene::evaluate(&scene.molecules[0].system, &rep.sel_text)
